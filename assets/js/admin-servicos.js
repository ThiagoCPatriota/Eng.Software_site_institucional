...
const list = document.querySelector("[data-servico-list]");
const empty = document.querySelector("[data-servico-empty]");
const filter = document.querySelector("[data-servico-filter]");
const newButton = document.querySelector("[data-new-servico]");
...
function resetForm() {
  ...
  form.elements.tipo.value = "biblioteca"; form.elements.ordem.value = "0";
...
function fillForm(item) {
  ...
  form.elements.tipo.value = text(item.tipo || "biblioteca");
...
function getPayload() {
  ...
  return {
    ...
    link_label: String(formData.get("link_label") || "").trim() || "Acessar serviço",
...
function getFilteredItems() {
  ...
  if (["biblioteca", "atividades_complementares", "tcc", "outro"].includes(value)) return items.filter((item) => item.tipo === value);
...
function renderItems() {
  ...
  visibleItems.forEach((item) => {
    ...
    meta.textContent = `${item.tipo || "serviço"} · ordem ${item.ordem ?? 0}`;
    ...
    summary.textContent = item.descricao || "Sem resumo cadastrado.";
...
async function loadItems() {
  let query = supabase.from("outros_servicos").select("*");
  query = query.order("ordem", { ascending: true }).order("criado_em", { ascending: false });
...
async function saveItem(event) {
  ...
  try {
    ...
    if (!payload.titulo || !payload.descricao || !payload.link_url) {
    ...
    if (id) {
      ...
      const { error } = await supabase.from("outros_servicos").update(nextPayload).eq("id", id);
      if (error) throw error;
      setStatus("Registro atualizado com sucesso.", "success");
    } else {
      const { error } = await supabase.from("outros_servicos").insert({ ...payload, criado_por: currentUser?.id || null });
      if (error) throw error;
...
async function updateItem(id, patch, message) {
  const { error } = await supabase.from("outros_servicos").update({ ...patch, atualizado_por: currentUser?.id || null }).eq("id", id);
  if (error) throw error;
